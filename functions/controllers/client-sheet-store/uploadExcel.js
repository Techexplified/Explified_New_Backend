...
fileUpload.get("/uploads", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error fetching file content", details: err.message });
  }
});
...
fileUpload.post("/upload", async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: "Error parsing form: " + err.message });
  }
  ...
  try {
    ...
    let fileContent;
    if (isCSV || uploadedFile.mimetype === "text/csv" || uploadedFile.mimetype === "text/plain") {
      // Parse CSV
